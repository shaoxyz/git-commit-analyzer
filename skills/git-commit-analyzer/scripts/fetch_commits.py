...
def fetch_local_commits(
    repo_path: str, since: str, until: str | None = None
) -> list[dict]:
    ...
    for line in output.strip().split("\n"):
        ...
        files_output = run_git_command(
            ["show", sha, "--name-status", "--pretty=format:"], repo_path
        )
        changed_files = []
        ...
        numstat = run_git_command(
            ["show", sha, "--numstat", "--pretty=format:"], repo_path
        )
        additions = 0
        deletions = 0
...
def fetch_github_commits(repo: str, since: str, until: str | None = None) -> list[dict]:
        ...
        commits.append(
            {
                ...
                "diff": diff,
                "changed_files": [
                    {"status": "M", "path": d["new_path"]} for d in diff_data
                ],
                "stats": {
                    "additions": additions,
                    "deletions": deletions,
...
def main():
    ...
    parser.add_argument(
        "--since",
        required=True,
        help="Fetch commits since (e.g., '1 day ago', '2024-01-01')",
    )
    parser.add_argument("--until", help="Fetch commits until (optional)")
    parser.add_argument(
        "--output", "-o", default="commits.json", help="Output file path"
    )
